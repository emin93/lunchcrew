...
export const LAST_WORKSPACE_CODE_KEY = 'lunchcrew.last_workspace_code';
...
export const BUILD_LABEL = 'Build rewrite-nextjs';
...
export function todayDateUTC() { return new Date().toISOString().slice(0, 10); }
export function makeDeviceId() { return `dev_${Math.random().toString(36).slice(2)}${Date.now().toString(36)}`; }
export function normalizeDisplayName(name: string) { return (name || '').replace(/\s+/g, ' ').trim().slice(0, MAX_DISPLAY_NAME_LENGTH); }
...
export const storage = {
  get(key: string) { if (typeof window === 'undefined') return null; return window.localStorage.getItem(key); },
  set(key: string, value: string) { if (typeof window !== 'undefined') window.localStorage.setItem(key, value); },
